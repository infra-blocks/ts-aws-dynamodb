...
- Support for `consistentRead` on `GetItem`.

## [0.81.1] - 2026-03-14

### Changed

- Minor refactorings around the interfaces of update actions. Update actions now implement
the internal `ExpressionFormatter`

## [0.81.0] - 2026-03-14

### Added

- Support for `returnItemCollectionMetrics` on `WriteTransaction`.

## [0.80.0] - 2026-03-14

### Added

- Support for `returnItemCollectionMetrics` on `UpdateItem`.

## [0.79.0] - 2026-03-14

### Added

- Support for `returnItemCollectionMetrics` on `DeleteItem`.

## [0.78.1] - 2026-03-11

### Changed

- Moved the implementation of `ifDefined` to [@infra-blocks/tookit](https://www.npmjs.com/package/@infra-blocks/toolkit).
...
has now been corrected to not compile if the sort key is missing.
...
- Support for `returnConsumedCapacity` for `Query`.

## [0.74.0] - 2026-03-08

### Added

- Support for `returnConsumedCapacity` for `UpdateItem`.
...
- Support for `returnConsumedCapacity` for `DeleteItem`.
...
- Support for `returnConsumedCapacity` for `GetItem`.
...
- The `Params` suffix has been replaced by the `Input` suffix everywhere exported.

## [0.65.0] - 2026-02-23

### Added

- Support for `projection` option in the `Query` command.
...
- The `WriteTransaction` command and related output `WriteTransactionOutput`.

### Changed

- Renamed `WriteTransactionParams` to `WriteTransactionInput`.
- Write transactions now returns `WriteTransactionOuput`, an unusable empty object,
instead of `void`.
...
- The `UpdateItem` command and related output `UpdateItemOutput`.

### Changed

- Renamed `UpdateItemParams` to `UpdateItemInput`.
- Updating items now return `UpdateItemOutput`, an unusable empty object, instead of `void`.
...
- Parameterized `UpdateItemParams` on key type.

## [0.51.0] - 2026-02-16

### Added

- Exporting the `PutItemReturnValue` and `PutItemResult` types.

### Changed

- Parameterized `PutItemParams` in a similar fashion as the previous change.

## [0.50.0] - 2026-02-16

### Added

- Exporting the `DeleteItemReturnValue` and `DeleteItemResult` types.

### Changed

- Parameterized `DeleteItemParams` in a similar fashion as the previous change.
...
- The `TypeMap` utility to convert from `NativeType` to its Typescript equivalent.

### Changed

- Renamed `Index` for `KeyDefinition`, `IndexAttributeDefinition` for `KeyAttributeDefinition`, and
`IndexAttributeType` for `KeyAttributeType`.
...
- Now exporting the `ConditionCheckParams` as originally intended.
...
- The package.json `main` field to support old school CJS imports.

## [0.41.0] - 2026-01-10

### Added

- Re-exporting a few more types for convenience. Those are the types used to parameterize
the client at construction: `DynamoDbClientConfig` and `DocumentClientConfig`.

## [0.40.1] - 2026-01-02

### Fixed

- Moved the `@infra-blocks/checks` dependency to prod dependencies as it is now used
while transforming results.
...
[0.80.0]: https://github.com/infra-blocks/ts-aws-dynamodb/compare/v0.79.0...v0.80.0
[0.79.0]: https://github.com/infra-blocks/ts-aws-dynamodb/compare/v0.78.1...v0.79.0
[0.78.1]: https://github.com/infra-blocks/ts-aws-dynamodb/compare/v0.78.0...v0.78.1
[0.78.0]: https://github.com/infra-blocks/ts-aws-dynamodb/compare/v0.77.0...v0.78.0
