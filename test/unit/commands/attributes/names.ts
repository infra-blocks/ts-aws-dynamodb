...
export const namesTests = () => {
  suite("names", () => {
    suite(AttributeNames.name, () => {
      suite("substitute", () => {
        test("should throw for empty string", () => {
          const names = AttributeNames.create();
          expect(() => names.substitute("")).to.throw();
        });
        test("should throw for a empty path", () => {
          const names = AttributeNames.create();
          expect(() => names.substitute("hello..not.valid")).to.throw();
        });
        test("should throw for empty index", () => {
          const names = AttributeNames.create();
          expect(() => names.substitute("[]")).to.throw();
        });
        test("should throw for missing attribute before index", () => {
          const names = AttributeNames.create();
          expect(() => names.substitute("[4]")).to.throw();
        });
        test("should return the expected substitution for a field", () => {
          const names = AttributeNames.create();
          const substitute = names.substitute("field");
          expect(substitute).to.equal("#attr1");
        });
        test("should return the expected substitution for a nested field", () => {
          const names = AttributeNames.create();
          const substitute = names.substitute("field.nested");
          expect(substitute).to.equal("#attr1.#attr2");
        });
        test("should return the expected substitution for an indexed field", () => {
          const names = AttributeNames.create();
          const substitute = names.substitute("list[0]");
          expect(substitute).to.equal("#attr1[0]");
        });
        test("should return the same substitution for the same field", () => {
          const names = AttributeNames.create();
          const first = names.substitute("field");
          const second = names.substitute("field");
          expect(first).to.equal(second);
        });
        test("should return the same substitution for the same nested field", () => {
          const names = AttributeNames.create();
        ...
        test("should honor the literal option with a regular path", () => {
          const names = AttributeNames.create();
          expect(names.substitute("toto", { literal: true })).to.equal(
            "#attr1",
          );
        });
        test("should honor the literal option with a path containing dots", () => {
          const names = AttributeNames.create();
          expect(names.substitute("outer.inner", { literal: true })).to.equal(
            "#attr1",
          );
        });
        test("should honor the literal option with an indexed path", () => {
          const names = AttributeNames.create();
