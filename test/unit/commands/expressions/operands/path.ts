...
export const pathTests = () => {
  suite("path", () => {
    suite(path.name, () => {
      test("should correctly substitute the value", () => {
    ...
    suite(literal.name, () => {
      test("should correctly substitute the value", () => {
        ...
        expect(result).to.equal(
          names.substitute("big.toto", { literal: true }),
        );
        expect(result).to.equal("#attr1");
      });
    });
    suite(isPathInput.name, () => {
      const invalid = [
      ...
      for (const { name, value } of invalid) {
        test(`should return false for ${name}`, () => {
      ...
      test("should return true for path instance", () => {
